...
async function scan() {
    ...
    for (const reg of regions) {
        ...
        const conn = `postgresql://postgres.${project}:${pass}@${host}:6543/postgres`;
        const client = new Client({ connectionString: conn, ssl: { rejectUnauthorized: false }, connectionTimeoutMillis: 2000 });
        try {
            await client.connect();
            console.log(`🎉 MATCH FOUND! Connected to region: ${reg} (${host}:6543)`);
...
`;
            ...
            `);
            console.log('✅ App settings updated with commercial and orders inboxes!');
    ...
    console.log('Finished scanning AWS regions.');
}
