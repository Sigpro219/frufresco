-- Fix total_price column mismatch in proc_log_audit for the orders table.
...
BEGIN
    ...
    ELSIF TG_TABLE_NAME = 'orders' THEN
        v_module := 'ORDERS';
        IF TG_OP = 'DELETE' THEN
            v_details := jsonb_build_object('id', OLD.id, 'sequence_id', OLD.sequence_id, 'total', OLD.total, 'status', OLD.status);
        ELSIF TG_OP = 'UPDATE' THEN
            v_details := jsonb_build_object(
                'id', NEW.id, 
        ...
        ELSE
            v_details := jsonb_build_object('id', NEW.id, 'sequence_id', NEW.sequence_id, 'total', NEW.total, 'status', NEW.status);
        END IF;
