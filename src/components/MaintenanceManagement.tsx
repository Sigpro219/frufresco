        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            ...
            <div style={{ 
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '0.4rem', borderRight: '1px solid #F1F5F9', paddingRight: '1rem' }}>
                    <CompactStat 
                        label="Urgentes" value={stats.urgent} color="#EF4444" icon={<AlertTriangle size={16} />} 
                        active={filterStatus === 'urgent'} onClick={() => setFilterStatus(filterStatus === 'urgent' ? 'all' : 'urgent')} 
                    />
                    <CompactStat 
                        label="Próximas" value={stats.upcoming} color="#F97316" icon={<AlertCircle size={16} />} 
                        active={filterStatus === 'upcoming'} onClick={() => setFilterStatus(filterStatus === 'upcoming' ? 'all' : 'upcoming')} 
                    />
                    <CompactStat 
                        label="Al Día" value={stats.ok} color="#10B981" icon={<CheckCircle2 size={16} />} 
                        active={filterStatus === 'ok'} onClick={() => setFilterStatus(filterStatus === 'ok' ? 'all' : 'ok')} 
                    />
                    <CompactStat 
                        label="Total" value={stats.total} color="#0891B2" icon={<ClipboardList size={16} />} 
                ...
                <div style={{ display: 'flex', gap: '0.8rem', alignItems: 'center' }}>
                    ...
                    <div style={{ display: 'flex', gap: '0.4rem' }}>
                        ...
                        >
                            {showHistory ? <Calendar size={16} /> : <FileText size={16} />}
                        </button>
                        {!readOnly && (
                            <button 
                                ...
                                return (
                                    <tr key={t.id} style={{ borderBottom: '1px solid #F1F5F9', backgroundColor: status === 'urgent' ? '#FEF2F2' : status === 'upcoming' ? '#FFF7ED' : 'transparent' }}>
                                        <td style={{ padding: '1.2rem' }}>
                                             ...
                                             <div style={{ fontSize: '0.75rem', fontWeight: '900', color: '#0891B2', marginTop: '0.2rem', display: 'flex', alignItems: 'center', gap: '4px' }}>
                                                 <Truck size={14} /> {t.vehicle?.plate}
                                             </div>
                                         </td>
                                 ...
                                 <div key={log.id} style={{ backgroundColor: '#F8FAFC', padding: '1.2rem', borderRadius: '20px', border: '1px solid #E2E8F0', display: 'grid', gridTemplateColumns: '1.5fr 1fr 1fr 0.5fr', alignItems: 'center', gap: '1rem' }}>
                                     ...
                                     <div style={{ textAlign: 'right' }}>
                                         {log.attachments?.length > 0 ? (
                                             <a href={log.attachments[0]} target="_blank" rel="noreferrer" style={{ fontSize: '1.2rem', textDecoration: 'none', display: 'inline-flex', alignItems: 'center' }}>
                                                 <Image size={18} color="#0891B2" />
                                             </a>
                                         ) : '---'}
                                     </div>
                                 </div>
                 ...
                 <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(15, 23, 42, 0.8)', backdropFilter: 'blur(8px)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }}>
                     <div style={{ backgroundColor: 'white', width: '100%', maxWidth: '550px', borderRadius: '32px', overflow: 'hidden', padding: '2rem' }}>
                         <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '1.5rem' }}>
                             ...
                             <button onClick={() => setShowCloseModal(false)} style={{ background: 'none', border: 'none', cursor: 'pointer' }}>
                                 <X size={20} />
                             </button>
                         </div>
...
function CompactStat({ label, value, color, icon, active, onClick }: { label: string, value: number, color: string, icon: React.ReactNode, active: boolean, onClick: () => void }) {
     return (
         <div 
             onClick={onClick}
             style={{ 
         ...
         >
             <div style={{ color: color, filter: active ? 'none' : 'grayscale(0.5)', opacity: active ? 1 : 0.7, display: 'flex', alignItems: 'center' }}>{icon}</div>
             <div>
